...
class DockerController {
          ...
          for (const fileName of COMPOSE_FILE_NAMES) {
            ...
            try {
              ...
              if (code !== 'ENOENT' && code !== 'ENOTDIR') {
                console.error(`[DockerController] Failed to read ${sanitizeForLog(filePath)}:`, sanitizeForLog((err as Error)?.message ?? String(err)));
                break;
              }
            }
          }
  ...
  public async getContainersByStack(stackName: string) {
    ...
    try {
      ...
      if (stdout && stdout.trim() !== '') {
        try {
          ...
          try {
            ...
            console.error(`Docker Compose JSON Parse Error for ${sanitizeForLog(stackName)}:`, sanitizeForLog(stderr || (parseError as Error).message));
      ...
      console.error(`Docker Compose Error for ${sanitizeForLog(stackName)}:`, sanitizeForLog(execError.stderr || execError.message || 'unknown'));
  ...
  private async smartFallback(stackName: string, stackDir: string): Promise<any[]> {
    try {
      ...
      console.error(`Smart Fallback failed for ${sanitizeForLog(stackName)}:`, sanitizeForLog((fallbackError as Error)?.message ?? String(fallbackError)));
      return [];
    }
  }
  ...
  public async removeContainers(containerIds: string[]) {
    ...
    for (const id of containerIds) {
      try {
        ...
        console.error(`Failed to remove container ${sanitizeForLog(id)}:`, sanitizeForLog(error.message));
