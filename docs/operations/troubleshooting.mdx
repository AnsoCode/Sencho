...
**Cause:** The gateway waits up to 5 minutes for the remote node to come back with a new version. A timeout can happen if:

- The image pull is slow on the remote host (large image, slow connection)
- The container failed to restart after pulling
- The remote node's version cannot be detected (shows "unknown") and it is running a version older than v0.40.0 that does not report its process start time. See [First remote update always times out](#first-remote-update-always-times-out) for details
...
## First remote update always times out

**Symptom:** After triggering a remote update, the node successfully restarts with the new version, but the dashboard still shows **Timed out** after 5 minutes.

**Cause:** The gateway detects update completion in two ways: a version change, or a change in the node's process start time. Nodes running Sencho versions older than v0.40.0 do not report either signal reliably:

- **Version:** The node reports `"unknown"` because its version detection is broken (fixed in v0.39.3+).
- **Process start time:** The node does not include `startedAt` in its `/api/meta` response (added in v0.40.0).

Without either signal, the gateway cannot confirm the container restarted, so it times out.

**Fix:** This only affects the **first** update from a very old version. After the update completes (even though the dashboard shows "Timed out"), the remote node will be running the latest version with both detection signals. Future updates triggered from the dashboard will complete normally.

1. Click **Dismiss** on the timed-out badge to clear the state.
...
**Symptom:** After clicking **Update**, the status immediately shows **Failed** instead of **Updating**.

**Cause:** The gateway could not reach the remote node's update endpoint, or the remote node rejected the request. Common reasons include network issues, expired API tokens, or the remote node not supporting self-update.

**Fix:**

1. Hover over the **Failed** badge to see the specific error message.
2. Verify the remote node is online and reachable from the gateway.
3. Check that the API token configured for the node is still valid.
