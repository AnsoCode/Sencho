...
**Symptom:** You created labels in Settings but no label pills appear in the sidebar.

**Checks:**

1. **License tier**: Labels require a Skipper or Admiral license. Verify your tier in **Settings > License**.
2. **At least one label must exist**: The pill bar only appears when you have created at least one label in **Settings > Labels**.
...
2. **Rule is enabled**: Check that the route's toggle is turned on in **Settings > Routing**.
...
**Fix:** If your LDAP server uses a self-signed certificate, set `SSO_LDAP_TLS_REJECT_UNAUTHORIZED=false` or toggle off "Verify TLS certificate" in **Settings > SSO**. For production, install a trusted certificate instead.
...
**Fix:** Go to **Settings > SSO** and verify the provider has an **Active** badge and its toggle is enabled.
