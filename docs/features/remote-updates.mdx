...
The gateway monitors the remote node until it comes back online, then marks it as **Updated**. Completion is detected either by a version change or by a change in the node's process start time (indicating a container restart). The "Updated" badge clears automatically after about 60 seconds and the node returns to "Up to date" status.
...
<Frame>
  <img src="/images/fleet-view/fleet-node-timed-out.png" alt="Node Updates dialog showing a timed-out node with retry and dismiss buttons" />
</Frame>
