...
<Note>
  Docker sometimes creates files inside the stack directory as root (e.g. volumes, config caches). Sencho handles this automatically — if a normal deletion fails due to permissions, it uses Docker itself to clean up root-owned files. No manual intervention is needed in the standard setup.
</Note>
