...
Click **Minimize** to collapse the modal to a small status pill anchored in the top right corner of the screen. The pill shows the action and stack name with a color-coded status dot (cyan pulse for in-progress, green for success, rose for error). Click the pill to expand the modal back.
